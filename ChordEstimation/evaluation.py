...
if __name__ == '__main__':
    configs_directory = os.path.join(os.path.dirname(__file__), 'configs')
    configs = os.listdir(configs_directory)
    configs.reverse()
    print(configs)

    train_dataset = 'ChordEstimation/data/train_out.pkl'
    test_dataset = 'ChordEstimation/data/test_out.pkl'
    for i, config in enumerate(configs):
        print(f'\n\nStarting Model {i + 1} out of {len(configs)}')
        try:
            import torch
            torch.cuda.empty_cache()
            train_and_test(os.path.join(configs_directory, config), train_dataset, test_dataset)
        except:
